...
if __name__ == '__main__':
    ...
    parser.add_argument('--experiment', default='baseline_ctd')
    ...
    for dataset in ['epic', 'mimic', 'cumc']:
        ...
        for ckpt in ['2500', '3000', '3500', '4000']:
            ...
            cached_fn = os.path.join(
                f'/nlp/projects/summarization/bhc_data_cleanup/{args.pretrained_model}_weights',
                args.experiment, f'{dataset}_{ckpt}'
            )
            ...
            out_dir = cached_fn + f'_w_bertscore'
            ...
            visit_meta = {}
            data_dir = f'/nlp/projects/summarization/bhc_data_cleanup/mistral_inference/{dataset}_8192'
            ...
            for exid, pred in exid2preds.items():
                ...
                if os.path.exists(ex_fn) and not args.overwrite:
                    ...
                    batch = tokenizer(
                        [source, clean_pred], padding='max_length', truncation=True, pad_to_multiple_of=1024,
                        max_length=16384, return_tensors='pt'
                    )
